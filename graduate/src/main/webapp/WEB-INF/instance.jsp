...
<body>
	<nav class="breadcrumb">
		...
		实例 <span class="c-gray en">&gt;</span> 实例列表 <a
	...
	<div class="page-container">
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"> <a class="btn btn-primary radius"
				onclick="picture_add('添加实例','/project/instance_add')"
				href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加实例</a></span>
		</div>
			...
			<table
				...
				<thead>
					<tr class="text-c">
						...
						<th>实例ID</th>
						<th width="80">实例名称</th>
						<th width="100">映像名称</th>
						<th width="100">IP地址</th>
						<th>大小</th>
						<th width="150">状态</th>
						<th width="150">电源状态</th>
						<th width="100">操作</th>
					</tr>
				</thead>
				<tbody>
					<c:forEach items="${datalist}" var="da">
						<tr class="text-c">
							...
							<td>${da.image_name}</td>
							<td>${da.ip}</td>
							<td class="text-c">${da.flavor}</td>
							<td>${da.status}</td>
							<td class="td-status"><span
								class="label label-success radius">${da.vmState}</span></td>
							<td class="td-manage"><a style="text-decoration: none"
								class="ml-5" onClick="instance_del(this,'${da.ID}')"
	...
	<script type="text/javascript">
		...
		/*实例删除*/
		function instance_del(obj, id) {
			console.log(id);
			layer.confirm('确认要删除吗？', function(index) {
				$.ajax({
					type : 'POST',
					url : '/project/instance_delete',
					dataType : 'json',
					data : {
						"ID" : id
					},
